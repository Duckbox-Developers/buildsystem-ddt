...
$(D)/busybox: $(D)/bootstrap $(ARCHIVE)/$(BUSYBOX_SOURCE) $(PATCHES)/$(BUSYBOX_CONFIG)
	...
	$(CHDIR)/busybox$(BB_SNAPSHOT); \
		...
		$(BUILDENV) \
		$(MAKE) busybox ARCH=$(BOXARCH) CROSS_COMPILE=$(TARGET)- CFLAGS_EXTRA="$(TARGET_CFLAGS)" $(GCC15PARM); \
		$(MAKE) install ARCH=$(BOXARCH) CROSS_COMPILE=$(TARGET)- CFLAGS_EXTRA="$(TARGET_CFLAGS)" $(GCC15PARM) CONFIG_PREFIX=$(TARGET_DIR)
	$(REMOVE)/busybox$(BB_SNAPSHOT)
	$(TOUCH)
...
$(ARCHIVE)/$(E2FSPROGS_SOURCE):
	$(DOWNLOAD) https://kernel.org/pub/linux/kernel/people/tytso/e2fsprogs/v$(E2FSPROGS_VER)/$(E2FSPROGS_SOURCE)
